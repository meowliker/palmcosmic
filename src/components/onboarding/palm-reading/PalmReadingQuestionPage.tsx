...
import type { PalmReadingQuestionScreen } from "./palmReadingFlow";

interface PalmReadingQuestionPageProps {
  screen: PalmReadingQuestionScreen;
}

async function persistPalmAnswer(screen: PalmReadingQuestionScreen, answer: string) {
  ...
  await fetch("/api/palm-reading/answers", {
    ...
    body: JSON.stringify({
      ...
      currentRoute: `/onboarding/palm-reading/${screen.id.replace("palm-", "")}`,
      currentStep: screen.id,
    }),
  });
}

export function PalmReadingQuestionPage({ screen }: PalmReadingQuestionPageProps) {
  ...
  const handleSelect = async (answer: string) => {
    ...
    trackFunnelAction("palm_reading_answer_selected", {
      funnel: "palm_reading",
      route: `/onboarding/palm-reading/${screen.id.replace("palm-", "")}`,
      step_id: screen.id,
      answer,
    ...
    persistPalmAnswer(screen, answer).catch(() => {
      trackFunnelAction("palm_reading_answer_save_failed", {
        funnel: "palm_reading",
        step_id: screen.id,
      });
    });
