...
type PalmAnswers = Record<string, string>;

function cleanString(value: unknown): string | null {
  return typeof value === "string" && value.trim() ? value.trim() : null;
}

function normalizeAnswers(raw: unknown): PalmAnswers {
...
export async function POST(request: NextRequest) {
  try {
      ...
      .upsert(
        {
          ...
          primary_flow: "palm_reading",
          primary_report: "palm_reading",
          updated_at: nowIso,
        },
        { onConflict: "id" }
      );
      ...
      .upsert(
        {
          ...
          user_id: userId,
          email,
          current_route: cleanString(body?.currentRoute) || existingSession?.current_route || "/onboarding/palm-reading",
          current_step: cleanString(body?.currentStep) || existingSession?.current_step || "palm-reading",
          priority_area: "palm_reading",
          answers: mergedAnswers,
          onboarding_data: existingSession?.onboarding_data || {},
          source: existingSession?.source || "palm_reading_answers",
    ...
    console.error("[palm-reading/answers] Error:", error?.message || error);
    return NextResponse.json(
      { success: false, error: error?.message || "Failed to save palm reading answers" },
      { status: 500 }
    );
  }
}
