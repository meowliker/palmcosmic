...
type CompatibilityAnswers = Record<string, string>;

function cleanString(value: unknown): string | null {
  return typeof value === "string" && value.trim() ? value.trim() : null;
}

function normalizeAnswers(raw: unknown): CompatibilityAnswers {
...
export async function POST(request: NextRequest) {
  try {
      ...
      .upsert(
        {
          ...
          primary_flow: "compatibility",
          primary_report: "compatibility",
          updated_at: nowIso,
        },
        { onConflict: "id" }
      );
      ...
      .upsert(
        {
          ...
          user_id: userId,
          email,
          current_route: cleanString(body?.currentRoute) || existingSession?.current_route || "/onboarding/compatibility",
          current_step: cleanString(body?.currentStep) || existingSession?.current_step || "compatibility",
          priority_area: "compatibility",
          answers: mergedAnswers,
          onboarding_data: existingSession?.onboarding_data || {},
          source: existingSession?.source || "compatibility_answers",
    ...
    console.error("[compatibility/answers] Error:", error?.message || error);
    return NextResponse.json(
      { success: false, error: error?.message || "Failed to save compatibility answers" },
      { status: 500 }
    );
  }
}
