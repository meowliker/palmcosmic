...
export default function FuturePredictionEmailPage() {
  ...
  const getFuturePredictionAnswers = () => {
  ...
  const saveOnboardingSnapshot = async (normalizedEmail: string) => {
    ...
    const snapshotResponse = await fetch("/api/onboarding/snapshot", {
      ...
      body: JSON.stringify({
        ...
        currentRoute: "/onboarding/future-prediction/email",
        currentStep: "future-prediction-email",
        priorityArea:
          localStorage.getItem("palmcosmic_priority_area") ||
          localStorage.getItem("astrorekha_priority_area"),
        answers: getFuturePredictionAnswers(),
        source: "future_prediction_email",
        onboardingData: {
          gender,
          birthMonth,
          birthDay,
  ...
  const handleContinue = async () => {
    ...
    try {
      trackFunnelAction("future_prediction_email_submit_started", {
        funnel: "future_prediction",
        route: "/onboarding/future-prediction/email",
        step_id: "future_email",
      });
      ...
      trackLeadCaptured({
        ...
        route: "/onboarding/future-prediction/email",
        step_id: "future_email",
        lead_source: "future_prediction_email",
      });
      ...
      if (stateResponse.ok) {
        ...
        if (stateData?.hasPaid) {
          ...
          router.push("/onboarding/create-password?flow=future_prediction&recovered=true");
          return;
        }
      }
    } catch (error) {
      console.error("Future prediction email step failed:", error);
      trackFunnelAction("future_prediction_email_submit_failed", {
        funnel: "future_prediction",
        route: "/onboarding/future-prediction/email",
        step_id: "future_email",
      });
    ...
    router.push("/onboarding/future-prediction/paywall");
  };
      ...
      <div className="flex-1 flex flex-col px-6 pt-10 pb-6">
        <div className="text-center">
          ...
          <p className="mx-auto mt-7 max-w-[23rem] text-base leading-relaxed text-[#b8c7da]">
            Your personal data is safe with us. We&apos;ll use your email for updates, receipts, and account details.
          </p>
        </div>
