...
export default function Step13Page() {
  ...
  useEffect(() => {
    pixelEvents.viewContent("Forecast Accuracy Boost Step", "onboarding_step");
    trackFunnelAction("forecast_accuracy_boost_viewed", {
      route: "/onboarding/step-13",
      step_id: "forecast_accuracy_boost",
      start_percentage: 34,
      target_percentage: 67,
    });
  ...
  const persistSnapshot = async (userId: string) => {
    ...
    await fetch("/api/onboarding/snapshot", {
      ...
      body: JSON.stringify({
        userId,
        email,
        currentRoute: "/onboarding/step-13",
        currentStep: "forecast_accuracy_boost",
        answers: {
          forecastAccuracy: 67,
        },
        onboardingData: {
        ...
        source: "forecast_accuracy_boost_page",
      }),
    }).catch((error) => {
      console.error("[step-13] snapshot failed:", error);
    });
  };
  ...
  const handleContinue = () => {
    ...
    trackFunnelAction("forecast_accuracy_boost_continue", {
      route: "/onboarding/step-13",
      step_id: "forecast_accuracy_boost",
      user_id: userId,
      next_route: "/onboarding/step-14",
    });
    ...
    <motion.div initial="hidden" animate="visible" variants={fadeUp} className="flex min-h-screen flex-1 flex-col bg-[#061525] text-white">
      ...
      <div className="px-6 pb-4">
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-[#15314d]">
          <div className="h-full w-[96%] rounded-full bg-[#38bdf8]" />
        </div>
      </div>

      <div className="flex flex-1 flex-col items-center px-6 pt-8">
        <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-8 text-center text-xl font-bold md:text-2xl">
          Forecast accuracy
        </motion.h1>
      ...
      {showButton ? (
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }} className="onboarding-cta">
          <Button
