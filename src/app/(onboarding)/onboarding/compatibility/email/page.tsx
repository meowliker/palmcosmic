...
import { COMPATIBILITY_ANSWER_KEYS } from "@/components/onboarding/compatibility/compatibilityFlow";

const EMAIL_PATTERN = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export default function CompatibilityEmailPage() {
  ...
  const getCompatibilityAnswers = () => {
    if (typeof window === "undefined") return {};

    return COMPATIBILITY_ANSWER_KEYS.reduce<Record<string, string | null>>((answers, key) => {
      answers[key] =
  ...
  const saveOnboardingSnapshot = async (normalizedEmail: string) => {
    ...
    await fetch("/api/compatibility/answers", {
      ...
      body: JSON.stringify({
        ...
        answers: getCompatibilityAnswers(),
        currentRoute: "/onboarding/compatibility/email",
        currentStep: "compatibility-email",
      }),
    });
    ...
    const snapshotResponse = await fetch("/api/onboarding/snapshot", {
      ...
      body: JSON.stringify({
        ...
        currentRoute: "/onboarding/compatibility/email",
        currentStep: "compatibility-email",
        priorityArea: "compatibility",
        answers: getCompatibilityAnswers(),
        source: "compatibility_email",
        onboardingData: {
          gender,
          birthMonth,
          birthDay,
  ...
  const handleContinue = async () => {
    ...
    try {
      trackFunnelAction("compatibility_email_submit_started", {
        funnel: "compatibility",
        route: "/onboarding/compatibility/email",
        step_id: "compatibility_email",
      });
      ...
      trackLeadCaptured({
        ...
        route: "/onboarding/compatibility/email",
        step_id: "compatibility_email",
        lead_source: "compatibility_email",
      });
      ...
      if (stateResponse.ok) {
        ...
        if (stateData?.hasPaid) {
          ...
          router.push("/onboarding/create-password?flow=compatibility&recovered=true");
          return;
        }
      }
    } catch (error) {
      console.error("Compatibility email step failed:", error);
      trackFunnelAction("compatibility_email_submit_failed", {
        funnel: "compatibility",
        route: "/onboarding/compatibility/email",
        step_id: "compatibility_email",
      });
    ...
    router.push("/onboarding/compatibility/paywall");
  };
      ...
      <div className="flex-1 flex flex-col px-6 pt-10 pb-6">
        <div className="text-center">
          <h1 className="mx-auto max-w-[25rem] text-2xl md:text-3xl leading-tight font-semibold tracking-tight">
            Sign up to save your compatibility reading with PalmCosmic
          </h1>
