...
export default function FuturePartnerEmailPage() {
  ...
  const getFuturePartnerAnswers = () => {
    if (typeof window === "undefined") return {};

    const keys = [
      "priority_area",
      "future-partner-partner-type",
      "future-partner-love-language",
      "future-partner-relationship-values",
      "future-partner-ideal-date",
    ];
  ...
  const saveOnboardingSnapshot = async (normalizedEmail: string) => {
    ...
    await fetch("/api/future-partner/answers", {
      ...
      body: JSON.stringify({
        ...
        answers: getFuturePartnerAnswers(),
        currentRoute: "/onboarding/future-partner/email",
        currentStep: "future-partner-email",
      }),
    });
    ...
    const snapshotResponse = await fetch("/api/onboarding/snapshot", {
      ...
      body: JSON.stringify({
        ...
        currentRoute: "/onboarding/future-partner/email",
        currentStep: "future-partner-email",
        priorityArea: "future_partner",
        answers: getFuturePartnerAnswers(),
        source: "future_partner_email",
        onboardingData: {
          gender,
          birthMonth,
          birthDay,
  ...
  const handleContinue = async () => {
    ...
    try {
      trackFunnelAction("future_partner_email_submit_started", {
        funnel: "future_partner",
        route: "/onboarding/future-partner/email",
        step_id: "future_partner_email",
      });
      ...
      trackLeadCaptured({
        ...
        route: "/onboarding/future-partner/email",
        step_id: "future_partner_email",
        lead_source: "future_partner_email",
      });
      ...
      if (stateResponse.ok) {
        ...
        if (stateData?.hasPaid) {
          ...
          router.push("/onboarding/create-password?flow=future_partner&recovered=true");
          return;
        }
      }
    } catch (error) {
      console.error("Future partner email step failed:", error);
      trackFunnelAction("future_partner_email_submit_failed", {
        funnel: "future_partner",
        route: "/onboarding/future-partner/email",
        step_id: "future_partner_email",
      });
    ...
    router.push("/onboarding/future-partner/paywall");
  };
      ...
      <div className="flex-1 flex flex-col px-6 pt-10 pb-6">
        <div className="text-center">
          <h1 className="mx-auto max-w-[25rem] text-2xl md:text-3xl leading-tight font-semibold tracking-tight">
            Sign up to save your future partner reading with PalmCosmic
          </h1>
